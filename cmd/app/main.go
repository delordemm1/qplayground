...
func syncRedisRunsOnStartup(pool *pgxpool.Pool, runCache automation.RunCache) {
	...
	query := `
		SELECT id, automation_id, status, start_time, end_time, logs_json, output_files_json, detailed_report_url, user_journey_report_url, error_message, created_at, updated_at
	...
	for rows.Next() {
		...
		var logsJSON, outputFilesJSON, detailedReportURL, userJourneyReportURL, errorMessage pgtype.Text

		err := rows.Scan(
			&run.ID, &run.AutomationID, &run.Status, &startTime, &endTime,
			&logsJSON, &outputFilesJSON, &detailedReportURL, &userJourneyReportURL, &errorMessage, &createdAt, &updatedAt,
		)
		if err != nil {
