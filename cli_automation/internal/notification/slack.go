...
func (s *SlackNotifier) buildSlackMessage(message NotificationMessage, channelConfig map[string]interface{}) SlackMessage {
	...
	case "completed":
		...
		mainText = fmt.Sprintf("%s CLI Automation *%s* completed successfully!", statusEmoji, message.AutomationName)
	case "failed":
		color = "danger"
		statusEmoji = ":x:"
		mainText = fmt.Sprintf("%s CLI Automation *%s* failed!", statusEmoji, message.AutomationName)
	default:
		color = "warning"
		statusEmoji = ":warning:"
		mainText = fmt.Sprintf("%s CLI Automation *%s* finished with status: %s", statusEmoji, message.AutomationName, message.Status)
	}
	...
	attachment := SlackAttachment{
		Color:  color,
		Title:  fmt.Sprintf("CLI Automation Run Details"),
