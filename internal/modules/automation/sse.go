...
func (s *SSEManager) SendRunProgress(runID string, message RunProgressMessage) error {
...
func (s *SSEManager) SendRunStatusUpdate(runID, status string) error {
	return s.SendRunProgress(runID, RunProgressMessage{
		Type:   "status",
		RunID:  runID,
		Status: status,
	})
...
func (s *SSEManager) SendRunLog(runID, stepName, actionType, message string, duration int64) error {
	return s.SendRunProgress(runID, RunProgressMessage{
		Type:       "log",
		RunID:      runID,
		StepName:   stepName,
...
func (s *SSEManager) SendRunError(runID, stepName, actionType, errorMsg string) error {
	return s.SendRunProgress(runID, RunProgressMessage{
		Type:       "error",
		RunID:      runID,
		StepName:   stepName,
...
func (s *SSEManager) SendRunStep(runID, stepName string, currentStep, totalSteps int) error {
	progress := 0
	if totalSteps > 0 {
...
func (s *SSEManager) SendRunComplete(runID, status string, duration int64, outputFiles []string) error {
	return s.SendRunProgress(runID, RunProgressMessage{
		Type:     "complete",
		RunID:    runID,
		Status:   status,
...
func (s *SSEManager) SendRunOutputFile(runID, outputFile string) error {
	return s.SendRunProgress(runID, RunProgressMessage{
		Type:       "output",
		RunID:      runID,
